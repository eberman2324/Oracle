...
run_process()
{
   ...
   then
      ...
      ### Run expdp
       ...
       cat ${ORACLE_BASE}/.${ORACLE_SID}.pw |expdp PARFILE=${SCRIPTS}/export/$PARFILE DIRECTORY=data_pump_dir PARALLEL=4 DUMPFILE=$EXPDUMPFILE LOGFILE=${EXPLOGFILE}
...
if [ $ERROR_COUNT -gt 0 ]
  then
    MailIt "$ORACLE_SID - expdp impdp failed." $EMPTY_FILE $ORACLE_SID email
    MailIt "$ORACLE_SID - expdp impdp failed." $EMPTY_FILE $ORACLE_SID pager
fi
