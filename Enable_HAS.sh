...
LOGFILE=$LOGDIR/Enable_has_$1_$DATEVAR.out
...
log_console "Start Enable HAS  `uname -svrn` at `date` using $0 $*"
log_console " "
...
log_console "About to enable HAS"

log_console "  "
log_console "About to enable HAS on machine `hostname` "
log_console " "
echodo crsctl enable has   | tee -a $LOGFILE
if [ ${PIPESTATUS[0]} -eq 0 ]; then
  log_console " " 
  log_console "HAS has been enabled!"
  log_console " "
else
  log_console " "
  log_console "HAS Enable has failed!!!!"
  log_console " "
fi
...
#########################################################################################################################################
#
echo "subject: HAS on `hostname` is enabled!" 	>> $MSGFILE
...
rm $MSGFILE
log_console " "
log_console "Enable HAS  complete  `uname -svrn` at `date` using $0 "
log_console " "
