...
rcat_DBName=`echo $DBName |tr "[:lower:]" "[:upper:]"`
...
rman << EOF
connect target /
connect catalog ${rcat_DBName}/${RCATPASS}@${RCATDB}
set echo on
backup device type sbt tag 'BK_${rcat_DBName}_ARC_${BT}' archivelog all not backed up delete all input;
EOF
