...
TEMPLATE1=${JOB_BASE}/NEWUSER_MESSAGE3.INFO
...
cat $TEMPLATE1 | /bin/sed -e "s/DATABASE_USERNAME/$newuser/g" |
                /bin/sed -e "s/FORMAL_NAME/$fullname/g" > $JOB_WD/$newuser.info
...
mutt -s "New Oracle Database Account " -c ${dbaemailaddress} ${emailaddress} <  ${newuser}.info >> $send_email_script
#echo "#" >> $send_email_script
