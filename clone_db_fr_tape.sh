...
#
# Added by T.S.
# Sample Execution - nohup clone_db_fr_tape.sh HEPYMGR 1574 HEPYPRD 1574 xhepydbm21p 6144 <<< $'6\nadd_sys_password_here\n' &
#        Note: Log Size is being passed to get by the redo check but the actual log size used is specified in the cmdfile
###########################################################################################################################
...
case ${SOURCE_HOST: -1} in
    p)  ORACLEDBA_SYSTEM='xoraclddbm1p-EMCLDPRD' ;;
    *)  ORACLEDBA_SYSTEM='xoraclddbw1d-EMCLDDEV' ;;
esac

ORACLEDBAPASS=`ssh -a -o StrictHostKeyChecking=no -i ~oracle/.ssh/id_dsa_tdmorassh tdmorassh@pum.aetna.com retrieve --AccountName oracledba --SystemName $ORACLEDBA_SYSTEM --ReasonText "Clone database ${SOURCE_DB}_${SOURCE_HOST}" | cut -f2 | tr -d '[[:space:]]'`

PUPPETPASS=`ssh -a -o StrictHostKeyChecking=no -i ~oracle/.ssh/id_dsa_tdmorassh tdmorassh@pum.aetna.com retrieve --AccountName puppet --SystemName NMA_Oracle --ReasonText "Clone database ${SOURCE_DB}_${SOURCE_HOST}" | cut -f2 | tr -d '[[:space:]]'`
...
cat  $ORACLE_BASE/admin/$ORACLE_SID/sql/create_${ORACLE_SID}_app_service.sql > runthis.sql
...
LOGFILE=$LOGDIR/clone_db_${1}_fr_tape_${DATEVAR}.out
export RMAN_AGENT_HOME=/orahome/u01/app/ddboost
export BACKUP_HOST="winpdd0821_lf2"
export STORAGE_UNIT="/ora_dev_boost3540_restore/xhepydbm21q"
...
# Added by T.S.
if [ ! -f $HOME/tls/refresh/${1}/duplicate_${3}_fr_tape.rman ] ; then
   log_console " "
   log_console "Duplication Script $HOME/tls/refresh/$1/duplicate_$3_fr_tape.rman Not Found - Script Aborting"
   exit 1
fi
...
echo "set echo off ver off pages 0 trims on head off feed off
select REGEXP_SUBSTR (SYS_CONTEXT ('USERENV','ORACLE_HOME'), '[^/]+', 1, 6) from dual;
...
rman_job=duplicate_${SOURCE_DB}_fr_tape
...
rman_cmdfile=$HOME/tls/refresh/${ORACLE_SID}/duplicate_${SOURCE_DB}_fr_tape.rman
...
echodo rman cmdfile=${rman_cmdfile} log=${rman_logfile} >> $rman_outfile 2>> $rman_outfile
...
if [ $? -eq 0 ]; then
  ...
  # Next 25 lines commented by T.S.
