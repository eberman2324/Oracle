...
sqlplus -s <<EOF
/ as sysdba
@${SCRDIR}/add_indexes_UB92.sql
EOF

# If Error
if [ $? -ne 0 ] ; then
   echo "Error Encountered adding indexes"
   mailx -s "Error Encountered adding indexes" ${MAILIDS} < add_indexes_UB92.out
   exit 1
else
   mailx -s "adding indexes Completed" ${MAILIDS} < add_indexes_UB92.out
fi
...
# Change Permissions
chmod 600 add_indexes_UB92.out
