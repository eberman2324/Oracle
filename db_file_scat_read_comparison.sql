...
--            Average db_file_scattered_read time in old environment
--            Average db_file_scattered_read time in new environment
...
                   / ( (MAX (a.total_waits) - MIN (b.total_waits)) * 1000),
                   1)
                   new_avg_read_time_scat_millsec
           FROM dba_hist_system_event a,
                dba_hist_system_event b,
                v$database c
          WHERE     a.event_name = 'db file scattered read'
...
                AND a.dbid = b.dbid)
SELECT *
  FROM db_scattered_old, db_scattered_new
