...
sqlplus -s <<EOF
/ as sysdba
@${SCRDIR}/run_redef_1.sql
EOF

# If Error
if [ $? -ne 0 ] ; then
   echo "Error Encountered Redef Table"
   mailx -s "Error Encountered Redef Table" ${MAILIDS} < run_redef_1.out
   exit 1
else
   mailx -s "Redef Table Completed" ${MAILIDS} < run_redef_1.out
fi
