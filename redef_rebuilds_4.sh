...
sqlplus -s <<EOF
/ as sysdba
@${SCRDIR}/redef_rebuilds_4.sql
EOF

# If Error
if [ $? -ne 0 ] ; then
   echo "Error Encountered in redef rebuilds"
   mailx -s "Error Encountered redef rebuilds" ${MAILIDS} < redef_rebuilds_4.out
   exit 1
else
   mailx -s "Redef rebuilds Completed" ${MAILIDS} < redef_rebuilds_4.out
fi
...
# Change Permissions
chmod 600 redef_rebuilds_4.out
