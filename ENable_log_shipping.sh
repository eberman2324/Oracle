...
#
#  Descrioption:  This script is called to enable log shipping on the primary database in a DataGuard  
...
LOGFILE=$LOGDIR/ENable_transport__${NOW}__$1.out
...
log_console "Start Enable Log Shipping $1 on  `uname -svrn` at `date` using $0"
log_console " "
...
echo -e "ABOUT TO      edit database '$PRIMARY_DB' set state=transport-on       \n\n" | tee -a $LOGFILE
dgmgrl -echo / "edit database '$PRIMARY_DB' set state=transport-on" | tee -a $LOGFILE
echo -e "\n=================================================================================================================================\n" | tee -a $LOGFILE
...
@check_dg_parms_nosp
EOF
log_console "End Enable Log Shipping $1 on  `uname -svrn` at `date` using $0"
#####################################################################################################
