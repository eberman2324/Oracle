...
sqlplus -s <<EOF
/ as sysdba
@${SCRDIR}/move_AUDIT_LOG_ENTRY.sql
EOF

# If Error
if [ $? -ne 0 ] ; then
   echo "Error Encountered Moving Table to BifFile Tbs"
   mailx -s "Error Encountered Moving Table to BifFile Tbs" ${MAILIDS} < move_AUDIT_LOG_ENTRY.out
   exit 1
else
   mailx -s "Moving Table to BifFile Tbs Completed" ${MAILIDS} < move_AUDIT_LOG_ENTRY.out
fi
...
chmod 600 move_AUDIT_LOG_ENTRY.out
