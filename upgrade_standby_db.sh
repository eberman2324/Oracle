#!/bin/bash
######################################################################################################
#  upgrade_standby_db.sh is executed to upgrad a standby database. 
#
#  usage: $ . upgrade_standby_db.sh  <dbms_upgrade_version> <target_instance_name>  
#
#
#  Maintenance Log:
#  version 1.0 08/2019     R. Ryan     New Script 
#  version 1.1 09/26/2019  R. Ryan     Copied extproc.ora to new oracle home to support databases using Voltage
...
LOGFILE=$LOGDIR/upgrade_standby_db_$2_$DATEVAR.out
...
log_console "Start standby DB upgrade of $2 on  `uname -svrn` at `date` using $0"
log_console " " 
...
if test $? -eq 1; then
  ...
  log_console " "
  log_console "Oracle Instance is  not active...start it before attempting upgrade"
  exit 1
fi
log_console " "
...
if [ -d ${STD_DBMS_DIR}/app/oracle/product/${VERSION} ]; then
  log_console "Oracle DBMS software version $VERSION is installed, continuing with upgrade"
else
  log_console "Oracle DBMS software version $VERSION is not intalled. Ensure $VERSION is installed before attempting upgrade"
  exit 1
fi
...
if [ $? -eq 0 ]; then
  log_console "Database exists in ORATAB, continuing with upgrade apply"
else
  log_console "Database is not present in /etc/oratab, make sure $ORACLE_SID is present before performing upgrade"
  exit 1
fi 
...
if [ $? -eq 0 ]; then
  ADG=true
  log_console " "
  log_console "Standby database is configured with Active Dataguard, setting startoption to mount for upgrade......"
...
cp $ORACLE_HOME/dbs/*${ORACLE_SID}* $ORACLE_BASE/product/${VERSION}/db_1/dbs
if [ $? -gt 0 ] ; then
  log_console "ERROR ---> DBS file move  Failed"
  exit 1
fi
log_console " "
log_console "DBS files for ${ORACLE_SID} have been copied to $ORACLE_BASE/product/$VERSION/dbs" 
...
$ORACLE_BASE/product/$VERSION/db_1/bin/srvctl add database -d ${ORACLE_SID}_${SERVER_NAME} -oraclehome $ORACLE_BASE/product/$VERSION/db_1 -spfile  $ORACLE_BASE/product/$VERSION/db_1/dbs/spfile${ORACLE_SID}.ora -startoption "MOUNT" -role physical_standby  -instance $ORACLE_SID -diskgroup "${STD_DATA_DG:1},${STD_REDO1_DG:1},${STD_REDO2_DG:1}">> $LOGFILE
...
log_console " "
log_console "Upgrade of $ORACLE_SID  to  $VERSION  complete on  `uname -svrn` at `date` using $0 "
exit 0
