...
sqlplus -s <<EOF
/ as sysdba
@${SCRDIR}/gather_PROD_stats.sql
EOF

# If Error
if [ $? -ne 0 ] ; then
   echo "Error Encountered Moving Table to BifFile Tbs"
   mailx -s "Error Encountered Moving Table to BifFile Tbs" ${MAILIDS} < gather_PROD_stats.out
   exit 1
else
   mailx -s "Moving Table to BifFile Tbs Completed" ${MAILIDS} < gather_PROD_stats.out
fi
...
# Change Permissions
chmod 600 gather_PROD_stats.out
