...
@kill_running_stats_jobs.sql ${MMDDYYYY}
EOF

# If Error
if [ $? -ne 0 ] ; then
   echo "Error Killing Stats Running Sessions "
   #mailx -s "Error Encountered Killing Stats Running Sessions" ${MAILIDS} < KILL_STATS_SESSIONS_${MMDDYYYY}.out
   exit 1
else
   echo "Killing Stats Running Sessions Completed"
   #mailx -s "Killing Stats Running Sessions completed" ${MAILIDS} < KILL_STATS_SESSIONS_${MMDDYYYY}.out
   exit 0
fi
