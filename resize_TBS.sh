...
sqlplus -s <<EOF
/ as sysdba
@${SCRDIR}/resize_TBS.sql
EOF

# If Error
if [ $? -ne 0 ] ; then
   echo "Error Encountered Moving Table to BifFile Tbs"
   mailx -s "Error Encountered Moving Table to BifFile Tbs" ${MAILIDS} < resize_TBS.out
   exit 1
else
   mailx -s "Moving Table to BifFile Tbs Completed" ${MAILIDS} < resize_TBS.out
fi
