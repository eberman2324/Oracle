...
###### 3. CREATE GRP                                                      ######
###### 4. DISABLE KILL JOB and STATS JOBS (enable crontab_during_grp.lst) ######
################################################################################
...
echo "Starting disabling KILL JOB and STATS JOBS"
log_console "Starting disabling KILL JOB and STATS JOBS"
log_console " "
...
if [ $retn_code_1 -eq 0 ]
then
  echo "Disable of KILL JOB and STATS JOBS completed with Success"
  log_console  "Disable of KILL JOB and STATS JOBS completed with Success"  
  log_console " "
  mailx -s " PY GRP creation completed with Success" ${MAILIDS} < ${LOGFILE}
  exit 0
else
  echo "Error disabling KILL JOB and STATS JOBS!!!"
  log_console "Error disabling KILL JOB and STATS JOBS!!!"
  log_console " "
  mailx -s "PY GRP creation completed with Errors!!!" ${MAILIDS} < ${LOGFILE}
  exit 1
fi
