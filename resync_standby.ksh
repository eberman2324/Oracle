...
copy_pwfile_from_primary()
{ 
   typeset primary_pwfile_path=`get_param PRIMARY_PWFILE_PATH`

   scp $primary_pwfile_path $ORACLE_HOME/dbs/orapw${STANDBY_INSTANCE} > $TEMPFILE
...
duplicate_for_standby()
{
   ...
   typeset auxiliary_connect_string=`get_param ${STANDBY_INSTANCE}_USERNAME_PW`@${STANDBY_INSTANCE}
   ...
   rman 2>&1 > $TEMPFILE <<-EOF 
	...
	#connect auxiliary $auxiliary_connect_string
	connect auxiliary /
...
JOB=`basename $0`
#SCRIPTS=/orahome/allu01/aetna/scripts
