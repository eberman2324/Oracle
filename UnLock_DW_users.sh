...
if [ -f UnLock_DW_users.out ] ; then
   rm UnLock_DW_users.out
fi
...
spool UnLock_DW_users.out
EXECUTE aedba.UNLOCKUSER;
spool off
EOF
...
if [ $SQLRC -ne 0 ] ; then
   echo
   echo "Error Encountered UnLocking Users in Database ${DBName}"
   echo
   exit 1
fi
