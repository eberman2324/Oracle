...
sqlplus -s <<EOF
/ as sysdba
@${SCRDIR}/add_indexes_POSTAL_ADDRESS.sql
EOF

# If Error
if [ $? -ne 0 ] ; then
   echo "Error Encountered adding indexes"
   mailx -s "Error Encountered adding indexes" ${MAILIDS} < add_indexes_POSTAL_ADDRESS.out
   exit 1
else
   mailx -s "adding indexes Completed" ${MAILIDS} < add_indexes_POSTAL_ADDRESS.out
fi
...
# Change Permissions
chmod 600 add_indexes_POSTAL_ADDRESS.out
